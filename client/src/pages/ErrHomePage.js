            ...
            {data && data.length > 0 && data[0].Orders[0]
              ...
              data.map((orders, orders_index) => {
                return orders.Orders.map((order, order_index) => {
                  return <tr key={orders_index}>
                    {
                      Object.key(order).map((key, key_index) => {
                        return (
                          orders_index > 0 && key == "Order ID" ?
                            null :
                            <td key={`${order_index}${key_index}`} rowSpan={key == "Order ID" ? orders.Orders.length : null}>
                              {order[key]}
                            </td>
                        );
                      })
